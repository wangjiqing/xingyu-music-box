...
- 当前包含主题、音乐库、星语音库连接配置、星语音库联调、系统媒体库测试、歌词测试、数据管理、播放器说明与版本信息。
- 星语音库连接配置可保存服务地址和 Access Key；SK 只保存在本次运行内存中，不写入钥匙串或普通配置文件。
- 随机播放队列由 `PlayerViewModel` 维护，iPad 界面只触发上一首/下一首/模式切换，不保存随机队列状态。
